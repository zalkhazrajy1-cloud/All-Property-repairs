...
(function () {
  ...
  if (form) {
    ...
    function isPhone(v) { return v === '' || /^[0-9()+\-\s]{6,}$/.test(v); }
    ...
    form.addEventListener('submit', function (e) {
      ...
      [name, email, phone, message].forEach(function (f) { if (f) clearError(f); });

      if (!name.value.trim()) { setError(name, 'Please tell us your name.'); valid = false; }
      if (!email.value.trim() || !isEmail(email.value.trim())) { setError(email, 'Please enter a valid email address.'); valid = false; }
      if (phone && !isPhone(phone.value.trim())) { setError(phone, 'Please enter a valid phone number.'); valid = false; }
      if (!message.value.trim() || message.value.trim().length < 10) { setError(message, 'Please describe the job in a few words (min. 10 characters).'); valid = false; }

      if (!valid) {
        status.textContent = 'Please check the fields marked below and try again.';
      ...
      setTimeout(function () {
        status.textContent = 'Thank you — your enquiry has been received. Andreas will be in touch shortly.';
